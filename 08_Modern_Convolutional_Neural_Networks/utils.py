...
def get_parameter_count(model: nn.Module, dummy_tensor: torch.Tensor | None = None, total_nb_params:int = None, verbose: bool = True) -> int:
    """
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.LazyConv2d) or isinstance(module, nn.Linear) or isinstance(module, nn.LazyLinear):
    ...
    if verbose:
        ...
        if nb_parameter_conv > 0:
            print(f"Number of parameters for Conv layers: {nb_parameter_conv} ({100*nb_parameter_conv/nb_parameter_total:.2f}% of count)")
        if nb_parameter_linear > 0:
            print(f"Number of parameters for Linear layers: {nb_parameter_linear} ({100*nb_parameter_linear/nb_parameter_total:.2f}% of count)")
...
def get_flop_count(model: nn.Module, dummy_tensor: torch.tensor, total_nb_flops:int = None, verbose: bool = True) -> int:
    ...
    if verbose:
        ...
        if nb_flops_conv > 0:
            print(f"Number of flopss for Conv layers: {nb_flops_conv} ({100*nb_flops_conv/nb_flops_total:.2f}% of count)")
        if nb_flops_linear > 0:
            print(f"Number of flopss for Linear layers: {nb_flops_linear} ({100*nb_flops_linear/nb_flops_total:.2f}% of count)")
